...
export function heatColor(t: Theme, rate: number, max: number): { bg: string; fg: string; sub: string } {
  ...
  const base = r > 0 ? t.up : t.down;
  const alpha = Math.round(step * 255)
    .toString(16)
    .padStart(2, "0");
  const strong = step >= 0.62;
  return { bg: `${base}${alpha}`, fg: strong ? "#FFFFFF" : t.ink, sub: strong ? "rgba(255,255,255,0.8)" : t.muted };
}
...
const styles = StyleSheet.create({
  ...
  counts: { fontSize: 9, fontVariant: ["tabular-nums"] },
