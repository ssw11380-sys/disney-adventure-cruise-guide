...
export function MarketStrip({ selected, onSelect, dense = false, trailing }: { selected?: string; onSelect?: (code: string) => void; dense?: boolean; trailing?: React.ReactNode } = {}) {
  ...
  if (list.length === 0)
    return dense && trailing ? (
      <View style={[styles.wrap, styles.denseWrap, { backgroundColor: t.surface, borderBottomColor: t.line }]}>
      ...
      {list.map((i, n) => {
        ...
        return (
          <Pressable
            ...
            style={({ pressed }) => [
              dense ? styles.itemDense : styles.item,
  ...
  if (!dense) return <View style={[styles.wrap, { backgroundColor: t.surface, borderBottomColor: t.line }]}>{strip}</View>;
  return (
    <View style={[styles.wrap, styles.denseWrap, { backgroundColor: t.surface, borderBottomColor: t.line }]}>
      {strip}
      {trailing}
...
const styles = StyleSheet.create({
  ...
  // ── 넓은 창 맨 위 띠 (dense) ──
  denseWrap: { flexDirection: "row", alignItems: "stretch", minHeight: layout.stripH },
  fill: { flex: 1 },
