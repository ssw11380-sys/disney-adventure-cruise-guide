 ...
 * ※ 합칠 때 정리: tableRowH·tableHeadH·heatTileW·discoverNameMin 은 설계(SPEC)의 layout 값 rowH 44·headH 40·heatTileW 150·이름 칸 최소 146 과
 *   같은 값이다. 웨이브 B(잔고 표)가 layout 무리에 넣으면 통합할 때 이 넷은 그 이름을 쓰게 바꾸고 여기서 지운다 (기준값은 한곳에)
...
export const foldScreens = {
   ...
   * ("한화에어로스페이스" 가 한 줄에 들어가는 폭, 잔고 표 이름 칸 157 과 비슷하게)
   */
  discoverNameMax: 160,
  /** 테마 히트맵 타일 기준 폭: 칸 수 = 폭 ÷ 이 값 (넓은 창만. 좁은 창은 지금처럼 3칸) */
  heatTileW: 150,
  /** 넓은 창 테마·업종 목록 칸 수 (한 칸이 themeCellMin 이상일 때) */
  themeListCols: 2,
  /**
   * 테마 목록 한 칸 최소 폭 (글자 100%, 큰 글씨는 배율의 절반만큼 넓힘).
   * 폴드8 펼침 세로 704(한 칸 352)·펼침 가로 853·울트라 펼침 세로 859 모두 두 칸 — 한 줄 전체(704)로 두면 테마 이름과 등락률 상자가 450dp 넘게 벌어진다 (진단 13·33).
   * 칸이 좁아진 만큼 대표 종목은 칸에 들어가는 수만 보인다 (lib/discoverColumns fitThemeLeaders)
   */
  themeCellMin: 340,
  /**
   * 여러 칸 테마 목록에서 이름을 줄여 보이는 마지막 대표 종목이 지키는 이름 폭 (글자 100%, 큰 글씨는 배율만큼 넓힘):
   * 한글 세 글자 폭(말줄임이면 두 글자와 말줄임). 이만큼도 안 남으면 그 종목은 빼고 앞 종목까지만 보인다 (등락률은 늘 말줄임 없이).
   * 글자 폭 어림이 넉넉해서(한글 1em) 실제로는 이보다 넓게 남는다 — 40 이면 폴드8 펼침 세로 한 칸에서 자리가 약 95dp 남는데도 둘째 종목을 뺐다
   */
  themeLeaderNameMin: 34,
  /**
   * 설정 카드 두 칸 배치에서 한 칸의 최소 폭 (글자 100%, 큰 글씨는 글자 배율만큼 넓힘, 최대 140%) — 이보다 좁으면 지금처럼 한 칸.
   * 설계의 칸 폭 범위(약 310~430) 안이고, 토스 카드 '자동 동기화'처럼 값이 긴 줄의 이름이 단어 중간에서 끊기지 않는 폭.
   * 글자 100%: 폴드8 펼침 세로 704(칸 348)부터 두 칸 · 폭 600~687 창은 한 칸.
   * 글자 130%: 칸이 442 이상이어야 두 칸이라 울트라 펼침 가로(954)만 두 칸, 폴드8 펼침 가로·세로와 울트라 펼침 세로는 한 칸
   */
  settingsColMin: 340,
  /**
   * 설정 카드 한 칸의 최대 폭 (글자 100%, 큰 글씨는 글자 배율만큼 넓힘, 최대 140%) — 이름과 스위치 사이가 300dp 이하 (설계).
   * 폴드8 펼침 가로(칸 422)·울트라 펼침 세로(425)·울트라 펼침 가로(473) 모두 400 으로 줄고, 남는 폭은 두 칸 사이 간격으로만 (양 끝에 붙는다)
   */
  settingsColMax: 400,
  /**
   * 넓은 창 배치 기준선의 켜기·끄기 여유 (히스테리시스): 설정 두 칸·테마 목록 칸 수·히트맵 칸 수·비중 배치 단계(원 아래 / 원 옆 두 줄 / 원 옆 한 줄).
   * 팝업·자유 크기 창을 끌어 기준선 근처를 오갈 때 배치(와 칸 수가 바뀌면 새로 만드는 목록)가 번갈아 바뀌지 않게,
   * 바로 전 배치에서 이만큼 더 넘어야 바꾼다 (2단 켜기 840·끄기 816 과 같은 폭).
   * 좁은 창(휴대폰 배치)에 있는 동안은 바로 전 배치를 잊는다 — 접은 화면에서 펴면 처음 연 것과 같은 배치 (lib/useSticky)
   */
  hysteresis: 24,
  /** 비중 원 지름: 카드 폭·남는 높이에 맞춰 이 범위 안에서 (넓은 창) */
  donutMin: 120,
  donutMax: 180,
  ...
  legendPctW: 48,
  /**
   * 비중 카드 범례 이름 칸이 바라는 폭 ("한화에어로스페이스" 한 줄, 글자 배율만큼 넓힘).
   * 원 옆 한 줄 범례는 가장 작은 원(donutMin) 옆에 이만큼 남을 때만 (아니면 평가금액을 이름 아래로 내린 두 줄 범례).
   * 원은 이 폭을 먼저 남기고 남은 폭으로 정한다
   */
  legendNameIdeal: 110,
  /**
   * 비중 카드 범례 이름 칸의 최소 폭 (글자 100% 에서 한글 여섯 글자, 글자 배율만큼 넓힘) — 원 옆 범례가 이만큼도 못 남기면 원 아래 범례.
   * 큰 글씨에서 두 줄 범례로는 카드 4장이 한 화면에 안 들어갈 때, 한 줄 범례를 쓰는 기준이기도 하다 (이름만 말줄임)
   */
  legendNameMin: 72,
  /**
   * 범례 높이 어림 (글자 100%, 큰 글씨는 배율만큼): 범례 머리(제목) 32 · 한 줄 범례 한 줄 26 · 두 줄 범례는 둘째 줄(평가금액 · 종목 수) 15 를 더함.
   * 원 크기와 촘촘한 범례를 고를 때 쓴다 (안드로이드 글꼴 줄 높이 기준으로 넉넉하게 — 웹 미리보기는 한 줄 약 25)
   */
  legendHeadH: 32,
  legendRowH: 26,
  legendSubH: 15,
  /**
   * 비중 화면에서 카드 격자 말고 한 화면에 늘 있는 높이 어림 (시스템 막대 제외, 글자 100% · 큰 글씨는 배율만큼):
   * 화면 머리 56 + 요약 한 줄 약 68 + 요약·격자 사이와 격자 두 줄 사이 간격·선 20 + 고지(아래 여백 제외) 약 25 + 여유 7.
   * 웹 미리보기로 잰 실제 값(울트라 펼침 세로 171)보다 조금 크게 잡는다 — 작게 잡으면 격자 높이를 크게 보아 원이 커지고 넷째 카드가 화면 밖으로 밀린다.
   * 크게 잡으면 원이 작아지거나 범례가 촘촘해질 뿐이다
   */
  allocChromeH: 176,
  /** 비중 화면 넓은 창 요약 줄에서 설명 글이 받는 최소 폭 — 총액 옆에 이만큼 안 남으면(큰 글씨) 설명이 다음 줄로 내려가 총액이 잘리지 않는다 */
  allocNoteMin: 240,
} as const;
