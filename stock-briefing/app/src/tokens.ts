...
export const foldScreens = {
   ...
   * 한글 세 글자와 말줄임. 이만큼도 안 남으면 그 종목은 빼고 앞 종목까지만 보인다 (등락률은 늘 말줄임 없이)
   */
  themeLeaderNameMin: 40,
  /** 설정 카드 두 칸 배치에서 한 칸의 최소 폭 (글자 100%, 큰 글씨는 배율의 절반만큼 넓힘) — 이보다 좁으면 지금처럼 한 칸 */
  settingsColMin: 300,
  /**
   * 설정 카드 한 칸의 최대 폭 (글자 100%, 큰 글씨는 배율의 절반만큼 넓힘) — 이름과 스위치 사이가 300dp 안팎으로 가깝게.
   * 폴드8 펼침 가로(칸 422)·울트라 펼침 세로(425)는 그대로, 울트라 펼침 가로(473)만 줄고 남는 폭은 칸 사이·양옆 여백에 고루 나눈다
   */
  settingsColMax: 430,
  /**
   * 넓은 창 배치 기준선의 켜기·끄기 여유 (히스테리시스): 설정 두 칸·테마 목록 칸 수·히트맵 칸 수·비중 원 옆 범례.
   ...
   * 가장 작은 원(donutMin) 옆에 이만큼 남을 때만 원 옆 범례, 아니면 원 아래 범례. 원 옆이면 원은 이 폭을 먼저 남기고 남은 폭으로 정한다
   */
  legendNameIdeal: 110,
  /** 원 아래 범례의 높이 어림 (글자 100%, 큰 글씨는 배율만큼): 범례 머리(제목) 32 · 범례 한 줄 26 — 남는 높이로 원을 키울 때 쓴다 */
  legendHeadH: 32,
  legendRowH: 26,
  /**
   ...
   * 웹 미리보기로 잰 실제 값(울트라 펼침 세로 171)보다 조금 크게 잡는다 — 모자라면 원이 작아질 뿐, 넉넉하면 넷째 카드가 화면 밖으로 밀린다
   */
  allocChromeH: 176,
} as const;
