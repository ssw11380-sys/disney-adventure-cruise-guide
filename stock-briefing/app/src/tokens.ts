...
export const layout = {
  ...
  shortHeight: 760,
  /**
   * 넓고 짧은 창(펼친 폴드8 가로)에서 탭을 왼쪽 세로 막대로 옮길지. 꺼 두면 펼쳐도 아래 탭 바 (2026-09-26 사용자 선택:
   * "펼쳤을 때 메뉴가 하단에 나오는 게 더 좋다") — 켜면 아래 railW·railHysteresis 규칙대로 다시 막대가 된다
   */
  railOn: false,
  ...
  divider: 1,
  /**
   * 종목·지수 상세 차트의 폭 상한. foldLayout 이 꺼져 있거나 좁은 창(휴대폰·접힌 화면)이면 3-42 이전 식 그대로
   * min(창 폭 − 56, 이 값) (lib/chartLayout phoneChartWidth — 사용자 결정 '접은 화면은 지금 그대로').
   ...
   * 200 = 가장 좁은 휴대폰(폭 360)의 차트 높이 (360 − 56) × 0.62 ≈ 188 보다 조금 높다 — 거래량(16%)·보조 지표(20%)를 모두 켜도
   * 가격 칸이 약 100dp 남는다 (200 − 날짜 줄 18 − 32 − 40 − 칸 사이 6 × 2 = 98). 단 그 폭의 폭 × chartAspect 보다 높이지는 않는다
   */
  chartMinH: 200,
  /**
   * 넓은 창 높이 상한(창 높이 × chartMaxHRatio)을 서서히 거는 폭 구간. 창 폭 mediumMin(600)에서는 상한을 쓰지 않고(좁은 창처럼 폭 × 0.62),
   * 600 + 96 = 696 에서 다 쓴다 → 팝업·분할 창을 끌어 폭이 599 ↔ 600 을 넘나들어도 높이가 355 → 200 처럼 뛰지 않는다 (폭 1dp 에 높이 약 2dp 이하).
   * 단 599 → 600 에서는 폭 식이 휴대폰 식(창 폭 − 56) → 잰 폭(창 폭 − 28)으로 바뀌어 폭 29·높이 약 18dp 가 한 번 달라진다.
  ...
  // ── 넓은 창 공통 치수 (3-42 통합: 웨이브 B·C·D·E 가 따로 두던 같은 값을 한 곳으로) ──
  // 한 줄 표의 줄(rowH 44)·머리(headH 44)·이름 칸 최소 폭(nameMinW 146)은 잔고 표와 발견 순위 표가 함께 쓴다
  /**
   * 옆에 붙는 두 번째 칸 폭 (글자 100%): 종목 상세 좌우 배치의 오른쪽 칸·울트라 펼침 세로 윗줄 오른쪽 칸(큰 글씨는 lib/detailLayout sideWidth 가
   * 늘어난 배율의 절반만큼 넓힌다), 넓은 창 브리핑 전체 화면 상세의 왼쪽 칸 (폭 '넓음')
   */
  detailSideW: 340,
  /** 브리핑 카드 격자(펼친 폴드8 세로) 한 칸 최소 폭 (큰 글씨는 배율만큼 넓힘) */
  briefCardMinW: 300,
 ...
 * 오른쪽 칸 폭은 브리핑 상세와 함께 쓰는 layout.detailSideW
 */
 ...
 * 카드 격자 한 칸 최소 폭·전체 화면 상세 왼쪽 칸 폭은 layout.briefCardMinW·layout.detailSideW
 */
export const foldBriefings = {
  /** 2단 목록 머리 (제목 · 변동 큰 순|등록순 · ⋯)·카드 격자 도구 줄·본문 도구 줄 높이 (표 머리 layout.headH 와 다른 것) */
  listHeadH: 44,
  ...
  /** 알약 한 칸 최소 폭 */
  pillMinW: 56,
  /** 폭 '중간'(펼친 폴드8 세로)에서 전체 화면 브리핑 상세 왼쪽 칸 폭 — 오른쪽 본문이 접은 화면 폭만큼 남게 (폭 '넓음'은 layout.detailSideW) */
  sideNarrowW: 300,
  /**
 ...
 * 순위 표의 줄·머리 높이, 이름 칸 최소 폭, 히트맵 타일 폭은 잔고 표와 같은 layout.rowH·headH·nameMinW·heatTileW (3-42 통합 때 한 곳으로)
...
export const foldScreens = {
  ...
  /**
   * 넓은 창 테마 보드 분포 줄 가운데 '가장 강한·약한' 두 칸의 최소 폭 (글자 100%, 큰 글씨는 배율만큼): 한 칸에 이름 세 글자 + 등락률 '+3.42%' 가 들어가는 폭.
   * 분포 글·정렬 버튼 옆에 이만큼 안 남으면(큰 글씨·좁은 창) 두 칸이 분포 줄 아래 줄로 내려간다 — 등락률이 옆 칸·정렬 버튼 위로 겹치거나 이름이 한 글자로 찌그러지지 않게
   */
  themeExtremesMinW: 260,
  /**
   * 설정 카드 두 칸 배치에서 한 칸의 최소 폭 (글자 크기와 상관없음) — 이보다 좁으면 지금처럼 한 칸. 설계의 칸 폭 범위(약 310~430) 안.
   * 폴드8 펼침 세로 704 부터 두 칸 · 폭 600~687 창은 한 칸. 큰 글씨(130%·140%)도 폴드8 펼침 가로·세로·울트라 펼침 모두 두 칸이고,
   * 좁은 칸에서는 토스 카드 '자동 동기화'처럼 값이 긴 줄의 값이 이름 아래 줄로 내려간다 (ui RowWrapContext — 이름이 단어 중간에서 끊기지 않게)
   */
  settingsColMin: 340,
  /**
   * 설정 카드 한 칸의 최대 폭 (글자 크기와 상관없음) — 이름과 스위치 사이가 300dp 이하 (설계).
   * 폴드8 펼침 가로·울트라 펼침 세로(칸 약 415)·울트라 펼침 가로(약 465) 모두 400 으로 줄고, 남는 폭은 두 칸 사이로만 (양 끝에 붙는다).
   * 430 이면 폴드8 펼침 가로에서 짧은 이름('평일만')과 스위치 사이가 316dp 로 벌어졌다 (통합 웹 미리보기 실측) → 400 에서 약 297
   */
  settingsColMax: 400,
  /**
