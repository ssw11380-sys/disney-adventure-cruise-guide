...
export async function redrawAllWidgets(data: WidgetData, now = Date.now()): Promise<void> {
  ...
  try {
    const [{ requestWidgetUpdate }, { renderBoth }, { WIDGET_NAMES }] = await Promise.all([import("react-native-android-widget"), import("./render"), import("./widgets")]);
    ...
    for (const name of [WIDGET_NAMES.holdings, WIDGET_NAMES.asset, WIDGET_NAMES.briefing, WIDGET_NAMES.market]) {
      await requestWidgetUpdate({ widgetName: name, renderWidget: (info: WidgetInfo) => renderBoth(name, data, { width: info.width, height: info.height, fontScale, now, pnlMode }) });
    }
  } catch {
