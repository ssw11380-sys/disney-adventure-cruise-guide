 ...
 *  - 새 서버: 그 판을 잃었으면(재시작·최근 5판 밖) 빈 쪽 + restart 를 준다. restart 를 안다고 알린 요청(r=1, api/client.ts)에만 —
 *    플래그가 없는 옛 앱에는 옛 서버처럼 지금 목록의 쪽을 준다 (옛 앱은 빈 쪽에서 더 보기·자동 갱신이 말없이 멈추므로)
 *  - 옛 서버: r 을 무시하고 알리지 않고 지금 목록의 쪽을 준다 → 받은 판이 요청한 판과 다르다
