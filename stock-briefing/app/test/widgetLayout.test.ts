...
const P_INDICES: IndexInput[] = [
  { label: "나스닥", value: "26,936.04", rate: "-1.13%", stale: false, tag: null },
  { label: "S&P500", value: "6,650.12", rate: "+0.19%", stale: false, tag: null },
  { label: "코스피", value: "3,412.35", rate: "+0.90%", stale: true, tag: "9/23" },
  { label: "코스닥", value: "862.15", rate: "-0.37%", stale: true, tag: "지연" },
  { label: "원/달러", value: "1,391.50", rate: "+0.38%", stale: false, tag: null },
];
...
function polishedOverflow(i: PolishedInput, p: PolishedPlan): string[] {
  ...
  if (p.total) {
    ...
    const w = (p.total.inline ? t + space.sm + pnl : Math.max(t, pnl)) + (p.compact ? space.sm + TOUCH : 0);
  ...
  if (p.index) {
    if (p.index.lines.length !== 1) bad.push("지수 줄이 두 줄");
    const line = p.index.lines[0]!;
    const w = line.reduce((a, it) => a + indexItemWidth(it, p.index!.font, s), 0) + indexSepWidth(p.index.font, s) * (line.length - 1);
    if (w > content) bad.push(`지수 줄 ${w.toFixed(0)} > ${content}`);
    // 앞에서부터 (뒤의 항목 — 환율 — 부터 뺀다)
    if (line.some((it, n) => it !== i.indices[n])) bad.push("지수 줄 순서");
  }
  if (p.note && textWidth(p.note, F.sm, s) > content) bad.push(`메모 ${p.note}`);
  const r = p.rows;
  i.rows.forEach((row, n) => {
...
describe("다듬은 잔고 위젯 배치 (widgetPolish): 숫자 잘림 0 · 높이 넘침 0 · 같은 크기에 종목 줄이 더 보인다", () => {
  ...
  it("지수 줄은 한 줄로, 좁으면 뒤(원/달러)부터 뺀다 — 흐린 값의 날짜('9/23')도 폭에 넣는다", () => {
    const wide = planHoldingsPolished(polishedInput(420, 300, 0.9, "cumulative", true, [])).index!;
    expect(wide.lines).toHaveLength(1);
    expect(wide.lines[0]![0]!.label).toBe("나스닥");
    const narrow = planHoldingsPolished(polishedInput(250, 300, 1, "cumulative", true, [])).index!;
    expect(narrow.lines[0]!.length).toBeLessThan(P_INDICES.length);
