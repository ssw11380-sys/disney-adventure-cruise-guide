...
describe("고지 문구", () => {
  ...
  it("브리핑 목록·상세·종목 상세 화면이 고지를 붙인다", () => {
    ...
    expect(read("app/src/components/BriefingBody.tsx").match(/<Screen[^>]*\bdisclaimer\b/g)?.length).toBe(3);
    expect(read("app/src/components/AccountBriefingBody.tsx").match(/<Screen[^>]*\bdisclaimer\b/g)?.length).toBe(2);
