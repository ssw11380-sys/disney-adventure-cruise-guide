...
/**
 * 위젯 2차: 폴드 위젯 크기 맞추기 (플래그 widgetFoldFit · widgetFoldBoth). 앱만 쓰는 기능이라 서버는 /api/widget 의 features 칸으로 전하기만 한다.
 * widgetFoldFit 을 끄면 features 에 false — 앱은 두 화면 크기 기억을 읽지도 적지도 않고 지금 그림 그대로 (app/test/widgetFold.test.tsx).
 * widgetFoldBoth(두 화면에 들어가는 카드)는 기본 꺼짐 — 폰에서 '한 그림을 두 화면이 같이 쓰는' 홈 화면으로 확인된 뒤에만 켠다
 */
describe("GET /api/widget: widgetFoldFit·widgetFoldBoth 는 features 칸으로만 전한다", () => {
  let db: Db | undefined;
  ...
  it("widgetFoldBoth 는 기본 꺼짐, 켜면 true — 응답의 다른 칸은 그대로", async () => {
    expect(FEATURES.widgetFoldBoth.default).toBe(false);
    await start();
    const urls = ["/api/widget", "/api/widget?indices=1&sessions=1&ui=2&board=1"];
    const off = await Promise.all(urls.map(get));
    await app!.inject({ method: "PUT", url: "/api/admin/features", payload: { widgetFoldBoth: true } });
    const on = await Promise.all(urls.map(get));
    urls.forEach((url, i) => {
      expect(off[i].features.widgetFoldBoth, url).toBe(false);
      expect(on[i].features.widgetFoldBoth, url).toBe(true);
      expect(on[i].features.widgetFoldFit, url).toBe(true);
      const strip = (b: { features: Record<string, boolean> }) => ({ ...b, features: { ...b.features, widgetFoldBoth: null } });
      expect(strip(on[i]), url).toEqual(strip(off[i]));
    });
  });
