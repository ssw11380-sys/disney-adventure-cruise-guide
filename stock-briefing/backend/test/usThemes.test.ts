...
describe("미국 테마 (토스 테마 분류 + 네이버 정규장 시세)", () => {
  ...
  it("개장 직후(오늘 거래대금이 거의 0)에도 직전 정규장 거래대금이 기준을 넘은 테마는 오늘·1주 목록에 남긴다", async () => {
    ...
    const mk = () =>
      new DiscoverService({ naver: new NaverDiscover(w.fetchFn), tics: new TossTics(w.fetchFn, 0), usThemes: new UsThemeBook({ tics: new TossTics(w.fetchFn, 0), naver: new NaverDiscover(w.fetchFn), now: () => now }), calendar, store, now: () => now });
    // 마감 직후 아직 OPEN 상태인 시세(끝나지 않은 값)는 기억하지 않는다
    for (const q of Object.values(w.quotes)) Object.assign(q, { marketStatus: "OPEN" });
    await mk().themes("US", "theme", "day");
    expect(saved.has("discover:us-theme-tv")).toBe(false);
    for (const q of Object.values(w.quotes)) Object.assign(q, { marketStatus: "CLOSE" });
    ...
    for (const q of Object.values(w.quotes)) if (!q.localTradedAt.startsWith("2025")) Object.assign(q, { localTradedAt: "2026-09-23T09:30:02-04:00", accumulatedTradingValueRaw: "100", marketStatus: "OPEN" });
    ...
    expect(day.note).toBe("오늘·직전 정규장 거래대금이 모두 100만 달러 미만인 테마 1개 제외");
    const week = await svc.themes("US", "theme", "week");
    expect(week.themes.map((t) => t.id)).toEqual(["956"]);
    // 개장 1시간 뒤부터는 오늘 값으로만 거른다 (마감 직전 15:50 기록본에 동전주 테마가 남지 않게)
    now = new Date("2026-09-23T14:31:00Z");
