...
describe("종목 뉴스 관련도 (3-12, 운영에서 뽑은 제목)", () => {
  ...
  it("이름이 흔한 낱말인 종목(대상·대교): 이름 검색 결과는 회사를 가리키는 꼴일 때만 남긴다 (재검토)", () => {
    ...
    expect(filterNews(daesang, found, NOW, true).map((x) => x.title)).toEqual(["대상, 3분기 영업이익 12% 증가", "대상그룹 임세령 부회장 승진", "㈜대상 청정원, 신제품 출시", "추석 선물세트 경쟁…CJ·대상·오뚜기 가격 동결"]);
    ...
    // 흔한 낱말이 아닌 두 글자 이름은 이름 검색에서도 그대로
    expect(filterNews({ code: "000270", name: "기아" }, [item("기아, 추석 시승 행사"), item("현대차·기아 EV 사수전")], NOW, true)).toHaveLength(2);
    ...
    expect(keep({ code: "004370", name: "농심" }, [item("들끓는 농심에 ‘농지 강제처분’ 유예"), item("추석 앞두고 '농심' 요동치자 진화나선 청와대"), item("농심, 대한항공 라운지 '라면 라이브러리'에 라면 단독 공급")])).toHaveLength(1);
  });
