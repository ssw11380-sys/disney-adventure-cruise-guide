 ...
 *  - indices: widgetIndexLine 이 켜져 있을 때만 지수 띠와 같은 목록(같은 인스턴스·30초 캐시·stale 규칙)에서 코스피·나스닥·원/달러.
 *    끄면 지수를 부르지도 않는다. 지수를 못 받으면 빼고 보낸다(위젯은 줄을 감춘다)
 */
export const widgetRoutes: FastifyPluginAsync<{ stocks: StockService; briefings: BriefingService; calendar: MarketCalendar; features?: FeatureService; indices?: MarketIndices }> = async (app, deps) => {
  ...
  app.get("/", async (req, reply) => {
    ...
    const indices = features.then((f) => (f?.widgetIndexLine && deps.indices ? deps.indices.list({ stale: true }).catch(() => null) : null));
