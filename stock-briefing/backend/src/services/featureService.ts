...
export const FEATURES = {
  ...
  widgetFoldFit: {
    default: true,
    description:
      "폴드 위젯 크기 맞추기 (위젯 2차, 앱만 — /api/widget features 로 전달): 위젯마다 바깥(접힘)·안쪽(펼침) 화면에서 본 크기를 기기에 기억해, 한 그림이 두 화면에 번갈아 보이면 두 화면에 모두 들어가는 크기로 그리고(남는 곳은 투명), 화면마다 따로 그리면 넓은 모습(평가금액 칸·두 열·지수 옆 칸)을 좁은 화면 폭이 허락할 때만. 앱이 떠 있을 때 접고 펴면 위젯을 다시 그림. 끄면 기억을 읽지도 적지도 않고 지금 그림 그대로",
  },
  widgetRefreshLog: {
    default: true,
    description:
