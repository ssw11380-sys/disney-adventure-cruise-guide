...
export const FEATURES = {
  ...
  widgetFoldFit: {
    default: true,
    description:
      "폴드 위젯 크기 맞추기 (위젯 2차, 앱만 — /api/widget features 로 전달): 접는 폰(짧은 변 600dp 이상 화면을 보인 기기)의 바깥 화면(600dp 미만)에서 그린 위젯과, 바깥 화면에서 본 적 있는 크기의 위젯에는 넓은 모습(잔고 평가금액 칸·두 열, 지수·환율 옆 칸)을 쓰지 않고, 두 화면에서 본 크기를 위젯마다 기기에 기억해 넓은 모습은 가장 좁은 폭이 허락할 때만. 일반 폰·태블릿은 지금 그림 그대로. 끄면 기억을 읽지도 적지도 않고 지금 그림 그대로",
  },
  widgetFoldBoth: {
    default: false,
    description:
      "(실험, widgetFoldFit 도 켜져 있어야 함) 한 그림을 두 화면이 같이 쓰는 홈 화면(① 범위 런처)으로 보이는 위젯을 두 화면에 모두 들어가는 카드(본 것 중 작은 폭 × 작은 높이, 남는 곳은 투명)로 그림. 두 화면에 따로 놓은 안쪽 위젯도 이렇게 보여 칸이 빌 수 있어, 설정 '화면 정보' 공유 글로 ①임을 확인한 뒤에만 켬",
  },
  widgetRefreshLog: {
    default: true,
    description:
