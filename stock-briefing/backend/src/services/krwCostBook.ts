 ...
 * 일시적인 조회 실패(429·네트워크·5xx)는 항목을 건드리지 않고 다음 동기화에서 다시 시도한다(몇 번이든).
 * 주문 내역이 보유 달러 매입금액을 설명하지 못하거나(이관 입고, 체결 순서가 엇갈린 주문, 주문 목록 지연) 그 시점 환율이 토스에 아예 없으면(404)
 * 기다리고(pending), PENDING_MS 넘게 계속되면 가진 정보로 보유와 맞춘다: 없는 환율·모자란 달러 매입금액은 표시 환율로,
 * 남는 매입금액은 비율대로 줄인다(추정 표시).
 */
...
/** 그 시점 환율이 토스에 아예 없다(404, 앞 시각들도 없음). 일시 오류와 달리 기다려도 생기지 않는다 */
...
export class KrwCostBook {
   ...
   * missingRate 가 있으면 토스에 아예 없는 과거 환율(RateNotFoundError)만 그 환율로 대신하고 approx 로 표시한다.
   */
  private async applyOrders(start: Position, orders: BookOrder[], missingRate: number | null = null): Promise<Position & { approx: boolean }> {
      ...
      if (o.side === "BUY") {
        ...
        if (i > lastZero) {
          try {
            ...
            if (!(err instanceof RateNotFoundError) || missingRate === null) throw err;
            rate = missingRate;
            approx = true;
          }
        }
   ...
   * (그래야 "이미 반영한 주문"이 저장하는 수량·매입금액과 정확히 짝이 맞는다).
   * 읽은 주문이 보유를 설명하지 못하면(주문 목록 지연일 수 있음) 건너뛰되, PENDING_MS 넘게 설명되지 않은 보유(이관 등)는 받아들인다.
   * 여러 계좌에 나눠 있으면 장부가 아는 계좌별 원화 금액(없으면 달러 매입금액) 비율로 나누고, 계좌별 값은 모르므로 추정으로 둔다.
   */
  setExact(values: Record<string, number>, read: () => Promise<AccountForBook[]>): Promise<SetExactResult> {
    return this.exclusive(async () => {
      const result: SetExactResult = { applied: [], skipped: [] };
      ...
      for (const [code] of codes) {
        ...
        if (!rows.length) {
          result.skipped.push({ code, reason: "not_held" });
          continue;
        }
        const out: Row[] = [];
        let skip: SetExactResult["skipped"][number] | null = null;
        for (const { account, h } of rows) {
          const key = keyOf(account, code);
          ...
          try {
            ...
            skip = { code, reason: "orders_failed" };
            break;
          }
          // 읽은 주문이 이 보유를 설명해야 "이미 반영"으로 기록할 수 있다 (주문 목록이 늦으면 빠진 체결이 영영 반영되지 않는다).
          // 오래(PENDING_MS) 설명되지 않은 보유는 이관 등으로 보고, 사용자가 넣은 값이 보유 전체의 값이 된다
          const target = h.purchaseAmount!;
          const pendingSince = state.pending[key] ? Date.parse(state.pending[key]) : null;
          const explained =
            ...
            (pendingSince !== null && now.getTime() - pendingSince >= PENDING_MS);
          if (!explained) {
            this.deps.log?.warn({ code, account, target }, "원화 매입금액 저장: 주문 내역이 아직 보유와 맞지 않아 건너뜀");
            state.pending[key] ??= seoulIso(now);
            skip = { code, reason: "unexplained", retryAfter: seoulIso(new Date(Date.parse(state.pending[key]!) + PENDING_MS)) };
            break;
          }
      ...
      for (const [code, rows] of prepared) {
        ...
        if (!stable) {
          ...
          result.skipped.push({ code, reason: "changed" });
          continue;
        }
  ...
  private async syncHolding(state: KrwCostBookState, account: number, h: HoldingForBook, displayFx: number | null): Promise<boolean> {
    ...
    // 환율 없이 먼저 판단한다: 1) 기존 항목에 새 체결만 이어 붙이면 맞는지 (토스 값·exact 몫이 그대로 남는다)
    // 2) 주문 내역 전체로 맞는지 (토스 값이 든 항목은 주문 목록이 잠깐 늦을 수 있어 오래 계속될 때만 바꾼다)
    // 3) 오래 설명되지 않으면(이관·체결 순서 등) 가진 값으로 보유에 맞춘다
    const contExplains = !!e && KrwCostBook.matches(KrwCostBook.usdAfter(e, orders), target);
    const rebuildExplains = KrwCostBook.matches(KrwCostBook.usdAfter({ quantity: 0, usdCost: 0, applied: {} }, orders), target);
    const plan = contExplains ? "cont" : rebuildExplains && (!e || e.krwExact <= 0 || overdue) ? "rebuild" : overdue ? "fit" : "wait";
    if (plan === "wait") return wait("주문 내역과 매입금액이 맞지 않음");
    const start: Position = plan === "rebuild" || !e ? EMPTY_POSITION : e;
    let p: Position & { approx: boolean };
    try {
      // 토스에 아예 없는 과거 환율은 오래 계속될 때만 표시 환율로 대신한다. 일시 오류(429·네트워크)는 몇 번이든 다시 시도
      p = await this.applyOrders(start, orders, overdue ? displayFx : null);
    } catch (err) {
      if (err instanceof RateNotFoundError) return wait("과거 환율 없음", { err: err.message });
      this.deps.log?.warn({ code: h.code, account, err: errText(err) }, "원화 장부: 환율 조회 실패, 다음 동기화에서 다시");
      return false;
    }
    if (plan === "cont") return write(p, !!e!.approx || p.approx);
    if (plan === "rebuild") return write(p, p.approx);
    // fit: 모자란 달러는 표시 환율로, 남으면 비율대로 줄인다
    const residual = target - p.usdCost;
    if (residual > 0) {
      if (!displayFx) return wait("표시 환율 없음");
      return write({ ...p, krwEst: p.krwEst + residual * displayFx }, true);
    }
    const k = p.usdCost > 0 ? target / p.usdCost : 0;
    return write({ ...p, krwExact: p.krwExact * k, krwEst: p.krwEst * k }, true);
  }
