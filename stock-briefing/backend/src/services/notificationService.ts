...
export class NotificationService {
   ...
   * 종목 브리핑이 하나도 없어도(모두 끈 종목·모델 장애) 계좌 브리핑으로 1건. 없으면 예전 그대로
   */
  readonly onSession = async (done: SessionDone & { account?: AccountBriefing | null }): Promise<void> => {
    ...
    const msg = buildDigest(done.session, done.date, items, digestAccount(done.account));
    if (!msg) {
