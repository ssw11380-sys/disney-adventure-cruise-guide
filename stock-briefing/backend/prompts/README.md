...
- `{{facts}}` (계좌 브리핑): 코드로 계산한 사실 목록(날짜·총 평가금액·당일 손익·기여 상위와 그 외·지수·원/달러와 환율 효과·오늘 일정·최근 공시와 건수, 오늘 한국 휴장·지난밤 미국 평일 휴장이면 '참고' 한 줄). `backend/src/services/accountNumbers.ts` 의 `factsText` 가 만듭니다.
  - 모델 설명의 숫자는 값·단위(원·%·포인트)·부호(적었으면)가 모두 맞는 숫자가 목록에 있어야 합니다. 날짜·시각(9/25·22:30)은 목록의 날짜·시각과만 맞춰 봅니다(장 시간 20:00 이 '20%'를 통과시키지 않게). 개수는 같은 낱말의 개수가 목록에 있어야 하고('9종목'은 목록의 'N종목'과, '6건'은 '최근 공시 N건'과), 순위 'N위'는 목록의 기여 순서 번호('1. 리게티 컴퓨팅')와 맞춰 봅니다. '4거래일'·'2배'처럼 모르는 말이 붙은 숫자는 목록의 단위 없는 같은 값(지수 값)과만 맞고, '번째'·'3일 연속' 같은 기간은 늘 거절합니다. 숫자 바로 뒤에 말로 적은 방향(올랐·상승·이익 ↔ 내렸·하락·손실)이 목록의 부호와 반대여도 버립니다('8.06% 올랐' ← 실제 -8.06%). 쉼표 자리가 틀린 숫자('-30,0890원')도 틀린 것으로 봅니다.
  - 매매 지시·행동 제안·전망 표현(매수·매도·추천·전망·예상·예측·가능성·반등·기대·비중·좋겠·고려해·권장·바람직·처분·차익·매집·기회·타이밍·앞으로·향후·당분간·'것으로 보'·'수 있습니다'·이어질·'내일도', 명령·권고형 어미 세요·십시오·시길·바랍니다 등)이 있어도 버립니다. 단 목록에 그대로 있는 공시 제목(예: 주식매수선택권부여에관한신고)을 옮겨 쓴 것은 괜찮습니다.
