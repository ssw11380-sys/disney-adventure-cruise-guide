...
- Railway 는 `railway.json` 의 `watchPatterns` 에 걸린 파일(`stock-briefing/backend/**`, `Dockerfile`, `.dockerignore`, `railway.json`)이 바뀐 커밋만 다시 배포합니다. 앱만 바꾼 PR 은 서버를 재시작하지 않습니다.
