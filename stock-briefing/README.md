...
- 테마·업종 상세의 상승·보합·하락 수는 목록(출처)과 같은 기준입니다(거래정지 제외). 구성 종목은 등락률 상위 300개까지 받으므로, 그보다 많은 업종은 "등락률 상위 300종목만" 안내가 붙습니다.
...
서버를 인터넷에 노출한다면 `.env` 에 `API_TOKEN` 을 반드시 설정하고(예: `openssl rand -hex 24`), 앱 **설정 탭 > 서버 주소 아래 토큰 칸**에 같은 값을 넣습니다. 설정하면 `/api/*` 전체가 `Authorization: Bearer <토큰>` 을 요구합니다(경로를 퍼센트 인코딩해도 라우터가 고른 경로로 판단합니다). `/health` 는 열려 있지만 토큰이 없으면 `ok`·시각·고지 문구만 주고, 상세(구독 종목·서버 IP·출처 구성)는 토큰을 보낸 요청에만 줍니다. 앱은 늘 토큰을 보내므로 그대로 동작합니다. 로컬 Wi-Fi 안에서만 쓸 때는 비워 두어도 됩니다.
