...
실시간: 키가 없어도 서버가 등록 종목 전체의 현재가를 토스 웹에서 요청 1개로 받아(2초 캐시) 돌려주고, 앱은 장 시간(평일 08시~다음날 07시, 한국·미국 장 포함)에는 홈·상세 화면에서 3초마다 다시 받아 HTS 처럼 가격이 움직입니다(장 밖에서는 1분, 앱이 뒤로 가면 멈춤). 가격이 바뀌면 오르면 빨강, 내리면 파랑으로 잠깐 깜빡입니다. 토스 Open API 키가 있으면 웹소켓 체결(`trade:kr`, `trade:us`)이 1순위로 덮어씁니다. 현재가 옆의 초록 점이 실시간 값이라는 표시입니다. 토큰은 클라이언트당 1개만 유효하므로 **같은 키를 다른 프로그램(챗GPT·클로드 연동 등)에 동시에 쓰면 서로 토큰을 무효화합니다** — 이 서버 전용 키를 쓰세요. 데이터는 토스증권 약관상 본인 매매 목적에 한해 쓸 수 있고 제3자 배포는 금지입니다.
