...
| 지수·환율 | 4×2 (가로·세로로 늘릴 수 있음) | 국내(코스피·코스닥) · 미국(나스닥·S&P500·다우·필라반도체) · 환율(원/달러·원/100엔·원/위안). 값·등락은 홈 지수 띠와 같은 표기, 장중은 초록 점, 출처가 늦으면 흐린 값 + "지연". 4×3 이상은 9개 모두, 4×2 는 6개. 누르면 그 지수·환율 차트. 플래그 `widgetMarket` |
